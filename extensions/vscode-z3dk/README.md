...
## VS Code Forks (Cursor, Antigravity)
The extension works in VS Code forks that support the VS Code extension API.

- **Cursor**:
  ```bash
  cursor --extensionDevelopmentPath=~/src/hobby/z3dk/extensions/vscode-z3dk
  ```
  Or install from a VSIX via the Extensions view ("Install from VSIX...").

- **Antigravity**:
  Use the same dev path workflow as VS Code, or install from VSIX if the fork
  blocks Marketplace installs. The extension is not tied to a specific store.

If you use a custom CLI name for your fork, replace `code`/`cursor` with that
binary name.

You can also package and install a VSIX directly:
```bash
cd ~/src/hobby/z3dk/extensions/vscode-z3dk
./scripts/package-vsix.sh
./scripts/install-vsix.sh cursor   # or antigravity
```
...
- `z3dk.projectRoot`: override project root (folder containing z3dk.toml).
- `z3dk.yazePath`: yaze binary path.
- `z3dk.yazeQuiet`: redirect yaze stdout/stderr to a log file (reduces spam).
- `z3dk.yazeLogPath`: log file path for yaze output.
...
- `z3dk.labelIndexPath`: path to the combined label index CSV (auto-detects if empty).
- `z3dk.labelMapPath`: path to the merged label map CSV (auto-detects if empty).
- `z3dk.labelIndexCommand`: command to regenerate label indexes.
- `z3dk.autoGenerateLabelIndex`: auto-regenerate label indexes after build/disasm/symbol export.
- `z3dk.labelIndexDebounceMs`: debounce delay before regenerating label indexes.
- `z3dk.labelSearchLimit`: maximum number of label search results to show.
- `z3dk.disasmCommand`: command to export USDASM-style disassembly (supports `${rom}`, `${symbols}`, `${output}`, `${usdasm}`, `${labels}`).
- `z3dk.disasmOutputPath`: output path for disassembly exports.
- `z3dk.autoOpenDisassemblyOutput`: auto-open disasm output after export.
- `z3dk.languageIds`: language ids that should activate z3lsp.
